...
Userscript v1.5.0 that reduces browser fingerprint uniqueness with lightweight per-session noise and defensive normalization based on real environment data.
...
- `clientRects` (`getBoundingClientRect` and `getClientRects`) with sub-pixel noise
...
- `Client Hints` (`navigator.userAgentData.getHighEntropyValues`) with version and model normalization
- `WebGPU` (`GPUAdapterInfo` via `adapter.info` and `requestAdapterInfo`)
...
| :--- | :--- | :--- |
| **Canvas 2D** | Sparse Rounding Noise | Applies sparse, near-zero-mean ±1 LSB variations that mimic natural GPU floating-point rounding anomalies instead of a constant offset (harder to flag statistically as tampered). |
...
| **Element rects** | Sub-pixel Noise | Adds sub-pixel variation to the width/height of `getBoundingClientRect`/`getClientRects` while preserving position (x/y/top/left) so layout is not affected. |
...
| **Client Hints** | `getHighEntropyValues` Entropy Reduction | Keeps brand and major version coherent but collapses build numbers (`platformVersion`, `uaFullVersion`, `fullVersionList`) and blanks the device `model`. |
| **WebGPU** | `GPUAdapterInfo` Normalization | Blanks the adapter's highest-entropy descriptors (`device` and `description`) while preserving coherent `vendor`/`architecture`, in both `adapter.info` and `requestAdapterInfo()`. |
...
- Canvas noise shaped like floating-point rounding (near zero-mean) instead of a constant offset
- Defensive normalization across multiple browser APIs, including Client Hints and WebGPU
...
- Privacy modes (`light`, `balanced`, `strict`) that adjust noise intensity and bucketing
- Sub-pixel noise on `getBoundingClientRect`/`getClientRects` without breaking layout
...
Since v1.4.0 the script ships three modes that centrally adjust bucketing granularity and noise intensity. The default mode is `balanced`.

| Mode | Screen / hardware bucketing | Canvas / WebGL / audio noise | `clientRects` noise | Best for |
| :--- | :--- | :--- | :--- | :--- |
| **light** | Fine (less information loss) | Soft | ~0.02 px | Maximum compatibility with sensitive sites |
| **balanced** | Medium | Medium | ~0.05 px | Balance between privacy and compatibility (default) |
| **strict** | Coarse (lower uniqueness) | Strong | ~0.1 px | Maximum entropy reduction |

To switch modes, use the **Low-Profile Fingerprint: modo (...)** command in the userscript manager menu. The value is persisted through `GM_setValue` and takes effect after a page reload.

> More aggressive modes reduce uniqueness at the cost of a higher chance of incompatibility. If a site breaks, switch to a lighter mode or use per-domain exclusion.
